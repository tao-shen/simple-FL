...
class DIN(nn.Module, Model_fn):
    def __init__(self, args):
        ...
        self.seq_embeddings = {}
        cat_size = 0
        for k,v in args.embedding.items():
            self.add_module(k, nn.Embedding(v['num'], v['size']))
        # self.item_embeddings = nn.Embedding(args.embedding['item']['num'], args.embedding['item']['size'])
        ...
        for key in self.features['cat_feats']:
            if 'item' in key:
                self.cat_embeddings[key] = getattr(self, 'item')
                cat_size += args.embedding['item']['size']
            else:
                self.cat_embeddings[key] = getattr(self, key)
                cat_size += args.embedding[key]['size']
        for key in self.features['seq_feats']:
            if 'item' in key:
                self.seq_embeddings[key] = getattr(self, 'item')
                cat_size += args.embedding['item']['size']
            else:
                self.cat_embeddings[key] = getattr(self, key)
                cat_size += args.embedding[key]['size']
        ...
        Model_fn.__init__(self, args, loss_fn=torch.nn.BCEWithLogitsLoss())
        # Model_fn.__init__(self, args, loss_fn=torch.nn.BCELoss())
